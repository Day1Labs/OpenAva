...
final class TeamSwarmCapabilityTests: XCTestCase {
    ...
    func testTeamToolDefinitionsExposeCoreSwarmTools() {
        ...
        XCTAssertEqual(byName["team_message_send"]?.permissionProfile, .internalCommunication)
        XCTAssertEqual(byName["team_plan_approve"]?.permissionProfile, .planApproval)
        XCTAssertEqual(byName["team_task_create"]?.permissionProfile, .taskUpdate)
        XCTAssertEqual(byName["team_task_update"]?.permissionProfile, .taskUpdate)
