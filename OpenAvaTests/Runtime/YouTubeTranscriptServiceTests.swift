...
final class YouTubeTranscriptServiceTests: XCTestCase {
    ...
    func testToolDefinitionExposesPageParameterInsteadOfSizeParameters() throws {
        ...
        let formatDescription = try XCTUnwrap(format["description"] as? String)

        XCTAssertEqual(page["type"] as? String, "integer")
        XCTAssertEqual(page["minimum"] as? Int, 1)
        XCTAssertEqual(format["default"] as? String, "transcript")
        XCTAssertTrue(formatDescription.contains("token-efficient plain subtitle text"))
        XCTAssertTrue(formatDescription.contains("use only when the user explicitly needs timestamps"))
        XCTAssertTrue(definition.description.contains("For content-only tasks, omit format"))
